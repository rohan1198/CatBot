...
def test_send_message(telegram_notifier, mock_bot):
    ...
    mock_bot.return_value.send_message.assert_called_once_with(chat_id="dummy_chat_id", text=message)
...
def test_send_photo(telegram_notifier, mock_bot):
    ...
    with patch('cv2.imencode') as mock_imencode:
        ...
        mock_bot.return_value.send_photo.assert_called_once_with(chat_id="dummy_chat_id", photo=np.array([1, 2, 3]).tobytes(), caption=caption)
