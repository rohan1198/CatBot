...
def test_update_motion_detector():
    ...
    dummy_frame = np.random.randint(
        0, 255, (480, 640, 3), dtype=np.uint8)  # Dummy frame for picam
    # Resized frame for cv2.resize, now 3-channel
    ...
    with patch('src.camera.Picamera2') as picam_mock, \
        ...
        resize_mock.assert_called_with(
            dummy_frame,
            camera.frame_size,
            interpolation=cv2.INTER_AREA)
        # Check if the returned frame is as expected
        assert np.array_equal(returned_frame, resized_frame)
